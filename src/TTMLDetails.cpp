...
void TTMLDetails::InitLayout() {
	...
	tableModel->setHorizontalHeaderItem(0, new QStandardItem(QString("trk.#")));
	...
	tableModel->setHorizontalHeaderItem(6, new QStandardItem(QString("TEXT")));
	tableModel->setHorizontalHeaderItem(7, new QStandardItem(QString("pTEXT")));
...
void TTMLDetails::rShowTTML(const QVector<visibleTTtrack> &rttmls, int search_time) {
	...
	for (int i = 0; i < ttmls->length(); i++) { // loop tracks
		...
		for (int z = 0; z < ttmls->at(i).elements.length(); z++) {
			...
			// create track number
			QStandardItem *track_nr = new QStandardItem(QString("%1").arg((ttmls->at(i).segment.track_index + 1)));
...
void TTMLDetails::pushButtonClicked() {
...
void TTMLDetails::showSelection(int track, int col, int item) {
	...
	case 5: // XML
		ttml_text->setLineWrapMode(QTextEdit::NoWrap);
		ttml_text->setFont(font_small);
		ttml_text->setPlainText(ttmls->at(track).segment.doc);
		break;
	case 6: // <HTML>
		render_style = 0;
		ttml_text->setFont(font_small);
...
void TTMLDetails::ClearTTML() {
	tableModel->setRowCount(0); // clear table rows
	ttml_text->clear();
	ttml_search_time->clear();
}

void TTMLDetails::prevClicked(bool) {
	emit PrevNextSubClicked(false);
}

void TTMLDetails::nextClicked(bool) {
