...
void SelectDeliverySpecificationPage::initializePage() {
	...
	if (wizard) {
		...
		if (!dsl_file.isEmpty()) {
			...
			props.schema_location("http://www.w3.org/XML/1998/namespace", (QApplication::applicationDirPath().append("/xsd/xml.xsd")).replace(" ", "%20").toStdString());
			props.schema_location("http://www.w3.org/2000/09/xmldsig#", (QApplication::applicationDirPath().append("/xsd/xmldsig-core-schema.xsd")).replace(" ", "%20").toStdString());
			props.schema_location("http://www.smpte-ra.org/schemas/433/2008/dcmlTypes/", (QApplication::applicationDirPath().append("/xsd/dcmlTypes.xsd")).replace(" ", "%20").toStdString());
			props.schema_location("http://www.imfug.com/ns/delivery-schema/2019", (QApplication::applicationDirPath().append("/xsd/IMF_DeliverySchema.xsd")).replace(" ", "%20").toStdString());
			try { dsl_data = dsl::parseDeliverySpecificationList(dsl_file.toStdString(),  0, props); }
