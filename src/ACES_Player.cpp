...
ACES_Player::ACES_Player() {
	...
	for (int i = 0; i < decoders; i++) {
		request_queue[i] = new ACES_FrameRequest();
		pointer_queue[i] = static_cast<QSharedPointer<ACES_FrameRequest>>(request_queue[i]);
		decoder_queue[i] = new ACES_Decoder(decoded_shared, pointer_queue[i], oetf_709, eotf_2020, eotf_PQ);
...
ACES_Player::~ACES_Player()
{
	timer->~QTime();
	for (int i = 0; i < decoders; i++) {
		request_queue[i]->~ACES_FrameRequest();
...
void ACES_Player::setPos(qint64 rframeNr, qint64 rTframeNr, int rplaylist_index) {
...
void ACES_Player::setPlaylist(QVector<VideoResource> &rPlaylist) {
...
void ACES_Player::setFps(int set_fps){
